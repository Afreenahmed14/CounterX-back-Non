...
public class PaymentServiceImpl implements PaymentService {
    ...
    // GST rate used to back-calculate subTotal and gstAmount
    // Must match the rate applied on the frontend (CartContext: tax = subtotal * 0.18)
    ...
    @Override
    @Transactional  // Rolls back cleanly if any step fails
    public void processPayment(Long orderId, String paymentMethod) {
        ...
        // Update order payment status and assign the daily token number now
        // that payment has actually succeeded.
        order.setPaymentStatus(PaymentStatus.PAID);

        Order lastOrder = orderRepository
                .findTopByOrderDateOrderByOrderIdDesc(LocalDate.now());
        int nextToken = (lastOrder != null && lastOrder.getDailyOrderNumber() != null)
                ? lastOrder.getDailyOrderNumber() + 1
                : 1;
        order.setDailyOrderNumber(nextToken);
        ...
        // NOTE: Daily revenue tracking and automatic bill generation are
        // intentionally left disabled here. Re-enable once their DTOs/fields
        // (subTotal, gstAmount) are wired up to match what the frontend sends.
    }
